...
- run #17 三个步骤全 **0s** ＝ 任务全部回放（对照冷跑 69 / 41 / 10s）；`Docs build` 从 20s 掉到 **1s**
  ...
  —— 这个 API 公开可读，判据可重跑。
- ⚠️ run #16 的 17s / 20s **不是**缓存失效：那次推送改了 admin 的一份**生成声明**
  （`components.d.ts` —— 当时它还**被 git 跟踪**，而 `.gitignore` 里那两条规则因为「带斜杠 = 锚定
  仓库根」其实**从未生效**；2026-09-24 已把它移到 `types/generated/` 并从跟踪面移除）⇒
  它在 admin 的 hash 输入里 ⇒ 改了就该真跑。**改了就真跑、没改就 0s**，这正是"按内容 hash
  判断"而不是"按这台机器上有没有"的现场证据。
  > 顺带一条**边界变化**：那两份文件移出跟踪面之后就不在 `$TURBO_DEFAULT$` 里了 ⇒
  > 它们不再是 hash 输入（正确：它们由 `src/**` 决定，而 `src` 本来就是输入）。
