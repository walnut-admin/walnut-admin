...
不用再在长串里找位置。实测 11 段的耗时合计约 68s，并行后墙钟约 27s（被最慢那段封顶）。
...
不在根 package.json 写复杂的 shell 脚本。所有跨包编排由 Turbo 处理，所有仓库级脚本（门禁、env 加解密、发版编排）由 `@walnut/scripts`（`packages/tooling/scripts/`，8 个 bin）与 `@walnut/release`（`packages/tooling/release/`，bin `walnut-release`）提供。根 scripts 保持"一句话委托"。
...
| [packages/tooling/scripts/](https://github.com/walnut-admin/walnut-admin/tree/main/packages/tooling/scripts) | `@walnut/scripts`——8 个 bin（`walnut-lint-workflows` / `walnut-setup-env` / `walnut-check-git-hooks` / `walnut-check-doc-refs` / `walnut-check-adr`） |
