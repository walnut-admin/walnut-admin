...
// root package.json — 极薄的一层
{
  "scripts": {
    ...
    // pre-push 的聚合门禁（lefthook 的 pre-push 只调这一条，十段按序跑）
    "prepush": "pnpm boundaries && pnpm lint:root && pnpm types:check && pnpm types:check:root && pnpm syncpack:lint && pnpm lint:workflows && pnpm lint:docs-refs && pnpm lint:adr && pnpm lint:doc-ts && pnpm change check",
...
pre-push   → pnpm --silent prepush（单条聚合门禁：十段按序跑，十秒级）
```

`prepush` = `pnpm boundaries && pnpm lint:root && pnpm types:check && pnpm types:check:root && pnpm syncpack:lint && pnpm lint:workflows && pnpm lint:docs-refs && pnpm lint:adr && pnpm lint:doc-ts && pnpm change check`
（架构边界 + **根级文件 lint** + 类型检查 + 根级配置类型检查 + 依赖一致性 + actionlint + 文档引用 + ADR 形态 + fixed 组版本锁步）。

`pre-commit` 只跑 ESLint fix，不做类型检查（太慢，阻塞 commit 体验）；`commit-msg` 由 commitlint 校验提交信息格式；架构边界（`turbo boundaries`）、根级文件 lint（`pnpm lint:root`）、类型检查、syncpack 依赖一致性、workflow 校验、两条文档闸门与版本锁步都放在 `pre-push`。pre-push 刻意收敛成**单条命令**：被截断时只会退化成"命令不存在"，响亮报错，而不是"语法合法但少跑几项"的静默弱化。
...
不在根 package.json 写复杂的 shell 脚本。所有跨包编排由 Turbo 处理，所有仓库级脚本（门禁、env 加解密、发版编排）由 `@walnut/scripts`（`packages/tooling/scripts/`，6 个 bin）与 `@walnut/release`（`packages/tooling/release/`，bin `walnut-release`）提供。根 scripts 保持"一句话委托"。
...
| [packages/tooling/scripts/](https://github.com/walnut-admin/walnut-admin/tree/main/packages/tooling/scripts) | `@walnut/scripts`——6 个 bin（`walnut-lint-workflows` / `walnut-setup-env` / `walnut-check-git-hooks` / `walnut-check-doc-refs` / `walnut-check-adr`） |
