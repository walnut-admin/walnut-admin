...
// root package.json — 极薄的一层
{
  "scripts": {
    ...
    // pre-push 的聚合门禁（lefthook 的 pre-push 只调这一条，九段按序跑）
    "prepush": "pnpm boundaries && pnpm lint:root && pnpm types:check && pnpm types:check:root && pnpm syncpack:lint && pnpm lint:workflows && pnpm lint:docs-refs && pnpm lint:adr && pnpm change check",
...
pre-push   → pnpm --silent prepush（单条聚合门禁：九段按序跑，十秒级）
```
...
不在根 package.json 写复杂的 shell 脚本。所有跨包编排由 Turbo 处理，所有仓库级脚本（门禁、env 加解密、发版编排）由 `@walnut/scripts`（`packages/tooling/scripts/`，5 个 bin）与 `@walnut/release`（`packages/tooling/release/`，bin `walnut-release`）提供。根 scripts 保持"一句话委托"。
...
| [packages/tooling/scripts/](https://github.com/walnut-admin/walnut-admin/tree/main/packages/tooling/scripts) | `@walnut/scripts`——5 个 bin（`walnut-lint-workflows` / `walnut-setup-env` / `walnut-check-git-hooks` / `walnut-check-doc-refs` / `walnut-check-adr`） |
