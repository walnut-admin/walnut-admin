...
| **R11** | **文档漂移清扫** | 小 | 2026-09-23 又清了一批（都按「引用了不存在的包名」机械扫出来）：<br>① `README.md` 的结构块**整块是虚构的** —— 列的 `packages/{shared,axios,core}` 三个包**一个都不存在**，且还在教人跑早已删除的 `pnpm dev:admin`；已按真实的 14 包 / 三分组重写，命令表也对齐现状。<br>② `CONTEXT.md`（术语表）把已更名的 `@walnut/axios` 当现存包（归档评审判定为过期）、且没有 tooling 5 包与 platform 分组；已校正，并在开头写明「只回答术语，结构与命令见 AGENTS.md」。<br>**剩余**：`.claude/`、`.zcode/`、`apps/server/.agents/docs/`、`apps/admin/.../AI/docs/` 等待查；`CONTEXT.md` 是否**并进文档站**（归档评审建议新建 `monorepo/glossary.md` 吸收它 —— 它现在**零引用**）留给 F1 一起定。 |
...
| **F2** | 补文档门禁（原 4 道，现剩 3 道） | — | ① `verify-md-links`（失效链接）→ ✅ **已由 VitePress 内置完成**（执行记录第 2 批）；<br>② **`verify-doc-refs`（拒绝引用已不存在的包名/路径）** ← **最值得做的一道**。2026-09-23 用一次性脚本扫出 15 类幽灵引用（`@walnut/axios` 35 处、`@walnut/tooling` 14 处，另有 `shared`/`core`/`ai`/`utils-core` 等），顺手查出**一个真 bug**（见执行记录第 5 批）与 README / CONTEXT 两处过期文档。<br>**但要先定口径**：ADR 与归档里的历史包名（"原 `@walnut/axios`"）**合法**，规划中未建的包（`@walnut/i18n` / `@walnut/security`）也**合法** —— 需要一份「允许提及」清单，否则门禁满屏误报。**建议**：只扫**非归档**的活文档 + 根入口文档，允许清单放「已更名/规划中」的包名。<br>③ `doc-typecheck`（fenced `ts` 块必须编译）：本仓文档里的 ts 块多是片段，成本可能高于收益。<br>④ `gen-package-catalog`（从 `package.json` 生成包清单）：手写清单散在 README / AGENTS / CLAUDE / `monorepo/index.md` / `turbo.md` 至少 5 处，本轮就修了其中 2 处。 |
