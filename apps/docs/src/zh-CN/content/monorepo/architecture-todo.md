...
| **前端启动序列没有容错**（症状是「白屏 / 一直转圈」） | 中 | `apps/admin/src/main.ts` 的 `await setupApp(app)` **在 `app.mount('#app')` 之前**，而 `setupApp` 里串着 Google 插件注册 → 指纹 → 设备 → 签名几组调用。**启动期任意一个 await 抛错，mount 就永不执行**：用户只看到 `apps/admin/index.html` 里那屏 loading，原因只在控制台。<br>**2026-09-24 实测两例**：① 空 `VITE_GOOGLE_CLIENT_ID` ⇒ `[GoogleSignInPlugin]: clientId is required to initialize`（已在 `apps/admin/src/App/src/scripts/google-signin.ts` 加「没配就跳过注册」的守卫）；② 后端起不来 ⇒ `/system/device/initial` 经代理 502 ⇒ `AxiosError` 同样卡在 splash。<br>**要做的判断**：这几组调用是「没它就不能进页面」还是「可以先进页面再补」—— 后者的 await 该挪到 `mount` 之后，或在组合根统一收敛错误。逐个改属业务代码，先在此登记。 |
| **env 模板里三个 `VITE_*` 是空串**（加密模板也是空的） | 小 | 2026-09-24 解开 `apps/admin/env-encrypted/.env`（只比对长度、不打印值）确认：`VITE_GA_ID` / `VITE_GOOGLE_CLIENT_ID` / `VITE_SERVER_STATIC_PUBLIC_KEY` 的**明文就是 `''`** ⇒ `pnpm setup-env` 恢复不出真值，本地要 Google 登录必须自己填。<br>**现状**：GA 由 `apps/admin/src/App/src/scripts/analytics.ts` 自己的 `if (!…VITE_GA_ID) return` 兜住；Google 已由上面那条守卫兜住；**`VITE_SERVER_STATIC_PUBLIC_KEY` 目前没人兜** —— `apps/admin/src/store/modules/user/user-auth.ts` 登录时拿它跟服务端下发的值比，空串大概率直接判不匹配（**未实测**，验证需要活的后端）。<br>**要决定**：模板是否该带一组可用的 dev 值（Google client id 属个人凭据、不该进仓），还是把「空值 ⇒ 该功能关闭」定成所有消费点统一的约定。 |
