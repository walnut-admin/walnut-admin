...
| **P1** | 静态检查与门禁的缺口 | **P1-17 · P1-18 · P1-19**（2026-09-23 交叉对比时新登记）<br><sub>R4 已修（`types:check:root`），R1 / R2 按决定搁置</sub> |
| **P2** | 维护性 / 体验改善 | R7 · A5 · A7 · A10 |
| **P3** | 远期 / 条件触发 | P3-12 · P3-13 · **P3-21** · **P3-23** · **P3-24** · **P3-25** · A8 · A9 · A11 · A12 · A13 |
...
> 是那次横向对比才看见的。所以「表是空的」从来不是目标，**目标是表里每一项都能说清它现在为什么不能做**。
>
> 剩下的每一项都属于下面四类之一：
>
> | 类别 | 含义 | 谁能让它动 |
...
> | **新登记的基建缺口** | 横向对比看见的"没人看这一面"（P1-17 / P1-18 / P1-19 / P3-23 / P3-24）。它们**能做**，只是没有比"已完成的 A/B 两批"更高的优先级 | 排期 |
...
原先的三条都已离场，**2026-09-23 又新登记三条**（来自[与参考仓 Z 的基建交叉对比](./reference-repo-comparison) 的 C 组，它们正是这个 P1 的典型形态：**不是"哪里写错了"，而是"没有任何东西在看这一面"**）：
...
| # | 事项 | 工作量 | 现状与判据 |
|---|------|--------|-----------|
| **P1-17** | **包标准的机械门禁（最小骨架）**（对比页 C2） | 中 | 参考仓有 **22 条**机械规则（`turbo-tags` / `exports-shape` / `script-matrix` / `package-json-fields` / 目录索引双向对账 …），本仓 **0 条** —— 对应物全是散文判据。**先做 `turbo-tags` 一条**：它约束的是各包 workspace 级 `turbo.json` 里的 tags，而**漏写一个 tag = 那个包静默豁免于整套 `turbo boundaries`**（边界门禁全绿，而它根本没被检查）。判据要写清"每个 workspace 包的 tags 必须落在枚举内且非空"，并**先实测一遍全部 workspace 包是否都合规**（不合规就得先补齐，否则门禁上不来）。 |
| **P1-18** | **根脚本的形态门禁**（对比页 C3） | 小 | 根 `package.json` 有 **46 条**脚本（2026-09-23 口径）、**零约束** —— 拼错一个名字只会在跑到它时报"命令不存在"。参考仓用「白名单 + 形态正则 + 理由」三件套；**本仓只搬前两段**（名字双向：`walnut-*` bin ↔ 根脚本必须两两对齐，`gate-wiring.test.ts` 的 B1 那段已经做了一半；形态正则：`lint:*` / `*:check` / `*:fix` 这类命名要有稳定形态），**不搬**它那三条 forbidden（本仓没有对应语境）。 |
...
| **P3-23** | **命令行公共层**（对比页 C1） | 大 | 参考仓有四件套：三态退出码归一（`verdict`）／前置条件错（`gate-ui` 的 `unmet()`）／统一参数解析（`argv`）／错误基类 + 唯一映射。本仓是**每道门禁各抄一遍 `return 2/0/1`**，以及一批裸 `console.*`（参考仓对应层只有 2 处）。<br>⚠️ **有绑定项，别只做一半**：若输出层改用 `stream.write`，必须同时把 `bin/*.ts` 那批 `process.exit(main())` 换掉 —— 否则会复现参考仓记录过的「管道下退出码对、拒绝理由一个字没出来」。<br>**为什么排 P3**：这是给**现有那批门禁**补一个共同层，收益是"少抄几遍"而不是"多看见一面"，所以排在 P1-17 / P1-18 这类"补上没人看的那一面"之后。 |
| **P3-24** | **turbo 缓存跨 CI run**（对比页 C4） | 中 | 实测全仓**没有任何 `actions/cache`** ⇒ CI 每次冷跑。⚠️ **风险与前置**：缓存 key 设计错会命中**陈旧缓存**，而 turbo 命中缓存时**门禁回放旧结论**（本仓已经在本地踩过这个形态：改文档不失效缓存导致死链校验被跳过）。所以**前置条件是先确认 hash 覆盖面**（哪些输入进了哈希、`TURBO_SCM_*` 与 env 是怎么算的），确认不了就别接。 |
