...
| **P2** | 维护性 / 体验改善 | R7 · A5 · A7 · A10 · **P2-22** |
...
**R8** · **F6** · **F2-a** · **F2③** · **F7** · **F8** · **P2-10** · **P2-21** · **P3-14** · **P3-15** · **P3-17** 已做完
...
> | **未裁决** | 需要人拍板（`F9` 组件文档哪棵树是正典 / 要不要英文站；`D1–D6`、`D8` 架构选型；`P2-22` 能不能改 `apps/server/libs/**` 下的文档） | 你 |
>
...
| **P2-22** | **`apps/server/libs/**` 下的文档没有任何门禁覆盖**（拆 P2-21 时顺带发现） | 小 | 那 9 份 `libs/*/README.md` 是**文档**，却落在「不碰 `apps/server/{apps,libs}`」这条铁律的路径里 —— 于是**没有任何门禁看它们**：`lint:doc-ts` 只扫 `apps/docs/src/`，别名检查只对 `.claude/skills/**` 开。<br>已经抓到一处实锤：`apps/server/libs/db/README.md` 里写着 `WalnutAdminConstDBModelName`（**全仓只剩这一处**，代码里早就叫 `WalnutDBModelName` 了）。<br>**要你定**：① 允许我改 `apps/server/libs/**` 下的**文档**（不碰任何代码）吗？允许的话我把那一处修掉，并把 `libs/**/*.md` 一并纳入别名检查；② 若不允许，就把铁律写清「libs 下的 README 也算业务代码」，我在「搁置」里记成「已知不动」。 |
...
| **F9** | **组件文档的「不可达」已修；剩下两件仍待裁决**（修 F2-a 时实测发现，此前无人记录） | ① ~~42 篇真实组件文档不在 sidebar~~ → ✅ **2026-09-23 已修**：`apps/docs/src/zh-CN/component/**` 的 **42 篇**按目录分成 6 组登记进 `.vitepress/config/zh.ts`（新增 `/component/` 这个**前缀键** —— 它与 `/content/` 是并列的两棵树，此前完全没有 sidebar，只能手敲 URL），标题逐个取自各页首个 `# `。机检：构建产物 `component/UI/button.html` 里已含同组其他页面的 sidebar 链接。<br>② `apps/docs/src/zh-CN/content/frontend/component/index.md` 在 sidebar 里，但按**另一套命名**（kebab-case、`advanced\|app\|business\|extra\|ui\|vendor`）规划了 74 个页面，**一个都没写** —— 与 ① 那棵树分类名、文件名大小写、目录层级全不一样。<br>③ `apps/docs/src/en-US/**` 44 篇（比 zh-CN 多 6 篇：`UI/{form,table,switch,timePicker,buttonGroup,inputNumber}.md`），但 locale 在 config 里**被注释掉**、不被服务；且它内容**是中文**（`# 表格`），并非英文站，只是没被服务的旧副本。`guide/deep/route.md` 的 en-US 版还有 GBK 误解码的中文乱码。<br>**仍待你裁决两件**：㊀ 哪棵树是正典 —— 在旧树上继续长，还是把 74 页按新命名写完再淘汰旧树？（① 只是先让旧树**可达**，没有回答这个问题）㊁ **要不要英文站** —— 这决定 `en-US/**` 是删、是修、还是继续冻结（归档评审第三档也留过同一问）；顺带决定 en-US 独有的那 6 篇要不要并进 zh-CN。 |
