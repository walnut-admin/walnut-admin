...
**最后核实**：2026-09-23 ｜ 15 个 workspace 包 ｜ 全门禁绿（`prepush` 门禁表 11 段 / `lint` / `types:check` / `test` / `boundaries` / `syncpack` / `build` / `build:docs` / `lint:adr`）
...
> ### 为什么表里还有条目（读之前先看这段）
>
> 剩下的每一项都属于下面三类之一 —— 换句话说，**「基建与文档」这一面已经清空**：
>
> | 类别 | 含义 | 谁能让它动 |
> |------|------|-----------|
> | **阻塞** | 等外部条件（全量 deps 升级、首次发版、stage 服务器…），每条都写了「解冻」条件 | 时间 / 外部环境 |
> | **业务代码类** | 要改 `apps/admin/src` 或 `apps/server/{apps,libs}` 的**实现**。这类按「本轮只做基建与文档」的决定**只登记不动手** —— 条目里那些**核实结论**就是留给将来那次改动的 | 先解除那条范围铁律 |
> | **未裁决** | 需要人拍板（`D1–D6`、`D8` 架构选型 —— 它们全部指向业务代码） | 你 |
>
> ⚠️ 所以「表是空的」不是目标 —— **目标是表里剩下的每一项都能说清「为什么它现在不能做」**。
>
...
> **本轮到此收尾**：从 2026-09-23 起逐批清理旧账，**「基建与文档」这一面已经清空** —— 剩下的每一项都落进了上面三类之一，没有一项属于它。
