...
10. **常驻文档有字数预算**：`pnpm lint:doc-budget` 盯着根 `AGENTS.md` / `CLAUDE.md` / 各 app 的
    `AGENTS.md` / 本页。**超上限红，用不到一半也红**（那种预算已经约束不到任何东西了）。
    所以往这几份里加内容前先问一句：这是**每次会话都要进上下文**的常驻规则，还是应该放文档站的
    参考资料？后者不该写在这里。
