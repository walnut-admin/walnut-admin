...
import { fileAtRef } from '@walnut/scripts/lib/git'
...
function isIntentFile(fileName: string): boolean {
 ...
 *
 * 只被本文件的两个判据使用（`consumedIntentIds` / `intentsOfVersion`），故不导出。
 */
...
/** 已被 ledger 记账的意图 id 集合（任何版本）—— 只服务本文件的两个判据，故不导出 */
function consumedIntentIds(): Set<string> {
 ...
 *
 * 只被下面 `unrelatedChanges()` 使用，故不导出（knip 会把没人 import 的 export 报成死代码）。
 */
function isNotReleaseOwned(file: string): boolean {
