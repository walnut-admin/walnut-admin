 ...
 * ## 曾经被排除、现在已纳入的最后一个
 *
 * `apps/server/AGENTS.md` 起初**不在**预算表里：它当时 16150 字符 / 537 行，是官方建议（单个指引
 * ≤200 行）的 2.5 倍，直接设预算会让门禁当场红、逼着在「加门禁」那一批里顺手做一次大拆分 ——
 * 那是**一次独立的文档重构**（已登记为待办 P2-21，排除这件事刻意写在注释与用例里而不是静默略过）。
 *
 * **2026-09-23 拆分完成**：常驻规矩留在原文件（157 行 / 6035 字符），参考型内容进文档站
 * `content/backend/` 的 5 篇（`architecture` / `dto` / `transactions` / `configuration` / `code-style`）。
 * 于是它现在**也进了预算表** —— 至此全仓每一份「每次会话都要进上下文」的指引都有上限。
 */
...
export const DOC_BUDGETS: readonly DocBudget[] = [
  ...
  {
    file: 'apps/server/AGENTS.md',
    maxChars: 6900,
    why: '**全仓最后一份被纳入预算的常驻指引**。它曾经 16150 字符 / 537 行（官方建议的 2.5 倍），2026-09-23 拆成「常驻规矩留本文件 + 参考型内容进文档站 `content/backend/` 五篇」后降到当前 6035（87%）。给上限是为了**防止它再把参考内容吸回来** —— 新内容先问一句：这是每次会话都要进上下文的规矩，还是该放那五篇里？',
  },
]
