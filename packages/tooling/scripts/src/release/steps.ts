...
import { assertSafeRef } from '../lib/ref-guard.ts'
 ...
 *
 * 为什么 `lint` 走 `turbo run lint lint:root` 而不是根 `pnpm lint`：根脚本只跑 `turbo lint`，
 * 而仓库级文件（根 `.mjs` / `.yaml` / `package.json`）的 lint 是**独立任务** `lint:root`，
 * 省掉它就会静默漏掉根级文件的 lint。
 *
...
const RELEASE_BATTERY: BatteryStep[] = [
  ...
  { id: 'lint', label: 'lint（含根任务 lint:root，不排任何包）', argv: ['exec', 'turbo', 'run', 'lint', 'lint:root'] },
 ...
 */
export async function runReleaseBattery(ui: ReleaseUi, skippedIds: Set<string>): Promise<void> {
 ...
 */
export async function runReleaseGates(ui: ReleaseUi, target: string, skipGates: SkipGates): Promise<void> {
  if (skipGates === 'all') {
